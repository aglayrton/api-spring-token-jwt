...
public record DadosCadastroMedicox(
  ...
  // Você pode criar um novo método na classe DadosCadastroMedicoxque retorna um objeto simplificado, contendo somente os campos que você deseja exibir.
  public SimplifiedMedico toSimplifiedMedico() {
    return new SimplifiedMedico(nome, email, crm, especialidade);
  }

  //  Essa classe foi criada como um registro interno, que é uma classe simples que contém somente campos e nenhum método adicional.
  public record SimplifiedMedico(
    String nome,
    String email,
    String crm,
    Especialidade especialidade
